...
void storePointWeights(wt_gmr<> pointWeights){
	this.pointWeights = pointWeights;
}
...
*/
/*! \file wm_int_weights.hpp
    \brief wm_int_weights.hpp contains a specialized class for a wavelet tree for
           sequences over large alphabets.
    \author Simon Gog
*/
#ifndef INCLUDED_SDSL_WM_INT_WEIGHTS
#define INCLUDED_SDSL_WM_INT_WEIGHTS
...
class wm_int_weights
{
    public:
        ...
        typedef random_access_const_iterator<wm_int_weights> const_iterator;
    ...
    protected:
        ...
        void copy(const wm_int_weights& wt)
        {
            m_size          = wt.m_size;
            m_sigma         = wt.m_sigma;
            m_tree          = wt.m_tree;
    ...
    public:
        ...
        //wt_gmr<> columnValues;//Added by jconcha: keeps weight of points in wmatrix, dictionary A[i]

		/*void fillPointWeights(WmPairs *pairs){
			pointWeights.resize(pairs->v_pair.size());
			
			for(int i = 0; i < pairs->v_pair.size();i++){
				pointWeights[i] = pairs->v_pair[i].column;
			}
		}*/
        ...
        /*! \param buf         File buffer of the int_vector for which the wm_int_weights should be build.
        ...
        wm_int_weights(int_vector_buffer<int_width>& buf, size_type size,
        ...
        //! Copy constructor
        wm_int_weights(const wm_int_weights& wt)
        {
            copy(wt);
        }

        //! Copy constructor
        wm_int_weights(wm_int_weights&& wt)
        {
            *this = std::move(wt);
        }

        //! Assignment operator
        wm_int_weights& operator=(const wm_int_weights& wt)
        {
            if (this != &wt) {
                copy(wt);
            }
        ...
        wm_int_weights& operator=(wm_int_weights&& wt)
        {
            if (this != &wt) {
                m_size          = wt.m_size;
                m_sigma         = wt.m_sigma;
                ...
                indexPositionsNonTerminals = wt.indexPositionsNonTerminals;
                indexPositionsColumn = wt.indexPositionsColumn;
            }
            return *this;
        }

        //! Swap operator
        void swap(wm_int_weights& wt)
        {
            if (this != &wt) {
                ...
                indexPositionsNonTerminals.swap(wt.indexPositionsNonTerminals);
                indexPositionsColumn.swap(wt.indexPositionsColumn);
            }
        }
        ...
        /*!
         * 
         * function to store weights to be returned in range_search_2d
         * 
         * 
         */
		void storePointWeights(wt_gmr<> pointWeights){
        ...
        size_type serialize(std::ostream& out, structure_tree_node* v=nullptr, std::string name="")const
        {
            ...
            //Test
            written_bytes += indexPositionsNonTerminals.serialize(out, child, "indexPositionsNonTerminals");
            written_bytes += indexPositionsColumn.serialize(out, child, "indexPositionsColumn");
        ...
        void load(std::istream& in)
        {
            ...
            indexPositionsNonTerminals.load(in);
            indexPositionsColumn.load(in);
            init_buffers(m_max_level);
        }
